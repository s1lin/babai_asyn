...
public:
	int n;
	double init_res, tol;
	vector<double> R_A, R_sA, y_A, raw_x0;
	Eigen::MatrixXd A, R;
	Eigen::VectorXd y, x0;

public:
	Babai_search_asyn(int n, bool f) {
		this->n = n;

		std::random_device rd;
		std::mt19937 gen(rd());  //here you could also set a seed
		std::normal_distribution<double> dis(-n, n);
		...
		if (f) {
			...
			std::mt19937 gen2(rd());
			std::normal_distribution<double> dis2(-10, 10);;
			this->A = MatrixXd::Zero(n, n).unaryExpr([&](int dummy) { return dis2(gen2); });
		...
		std::normal_distribution<double> dis3(-n, n);
		this->y = R * x0 + 0.5 * Eigen::VectorXd::Zero(n).unaryExpr([&](int dummy) { return round(dis3(gen)); });
		y_A.resize(y.size());
...
#pragma omp parallel reduction(+ : sum)
					...
					raw_x_A[n - 1 - i] = round((y_A[n - 1 - i] - sum) / R_sA[(n - 1 - i) * n - ((n - 1 - i) * (n - i)) / 2 + n - 1 - i]);
					sum = 0;
				}
			}
