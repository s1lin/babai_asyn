...
int main() {
    ...
    if (file.is_open()) {
        for (int init_value = -2; init_value <= 2; init_value++) {
            ...
            for (int n_proc = 5; n_proc < 100; n_proc += 5) {
                for (int i = 0; i < 10; i++) {
                    auto[omp_res, omp_time, num_iter] = bsa.search_omp(n_proc, nswp, init_value);
                    res[2] += omp_res;
                    tim[2] += omp_time;
                    itr[2] += num_iter;
                }
                file << init_value<< "," << n_proc << "," << res[2] / 10 << "," << tim[2] / 10 << "," <<itr[2] / 10 << ",\n";
