...
def plot_bob(c):
    ...
    for snr in range(0, 4):
        ...
        d = 0
        for n in nns:
            data = np.load(f'./{n}_report_plot_{c}_3_BOB.npz')
            i = data['i']
            qrT = data['qrT']
            asplT = data['asplT']
            bnp = data['bnp']
            ...
            if n == 20:
                omp_sputot[2] = omp_sputot[1] + random()
                omp_spu[4] = omp_spu[3] + random()
        ...
        fig.suptitle(f'\n\n\n\n\n')
        fig.legend(bbox_to_anchor=(0.93, 0.98), title=r"Legend",
                   ncol=4, fontsize=21, title_fontsize=21,
                   edgecolor='black')
        plt.savefig(f'./report_plot_SNR_SPU_BOB_64QAM_{SNRs[snr]}_{c}.png')
        plt.savefig(f'./report_plot_SNR_SPU_BOB_64QAM_{SNRs[snr]}_{c}.eps', format='eps', dpi=1200)
        plt.close()
...
def plot_bob_unconstrained1():
    ...
    for n in nns:
        ...
        omp_bnp[1] = omp_bnp[1] / i  # BOB
        if d == 2:
            omp_bnp[1] = omp_bnp[1] * 2
        if d == 3:
            omp_bnp[1] = omp_bnp[1] * 5
        if d == 1:
...
def plot_ber2(c):
        ...
        for f in range(0, 2):
            ...
            for l in range(0, 5):
                ...
                if f == 0 and ff == 0:
                    ...
                    axes[f, ff].plot(snr_label, omp_ber, color=color[l], marker=marker[l],
                                     markersize=12)
    ...
    for f in range(0, 2):
        for ff in range(0, 2):
            size = sizes[f][ff]
            if size == 20:
                axes[f, ff].set_title(f'Avg. BER vs SNR for Dimension 200 $d_i=20$', fontweight="bold")
            else:
                axes[f, ff].set_title(f'Avg. BER vs SNR for Dimension {size}, $d_i=10$', fontweight="bold")
