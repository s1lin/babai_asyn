...
def save_data(n, i, max_proc, min_proc, qrT, asplT, bnp, ber, itr):
...
def plot_bnp(n, i, max_proc, min_proc, qrT, asplT, bnp, ber, itr):
    np.savez(f'./test_result/{n}_report_plot_{i}_BNP.npz', n=n, i=i, max_proc=max_proc, min_proc=min_proc,
             qrT=qrT, asplT=asplT, bnp=bnp, ber=ber, itr=itr)
    print("\n----------PLOT RUNTIME--------------\n")
    color = ['r', 'g', 'b', 'm']
    marker = ['o', '+', 'x', '*']
    procs = [2, 4, 8, 16]
    labels = ['$x_{init} = round(x_R)$', '$x_{init} = 0$', '$x_{init} = avg$', 'Reduction']
    ...
    for j in range(0, 2):
        if j == 0:
            ...
            plt2.rcParams["figure.figsize"] = (12, 15)
            fig, axes = plt2.subplots(3, 2, constrained_layout=True)

        qam = 4 if j == 0 else 64
        axes[0, 0].set_title('Iterations vs Number of Cores')
        axes[0, 1].set_title('BER vs Number of Cores')
        axes[1, 0].set_title('Avg Time vs Number of Cores')
        axes[1, 1].set_title('Speed Up vs Number of Cores')
        axes[2, 0].set_title('Avg Time with Reduction vs Number of Cores')
        axes[2, 1].set_title('Total Speed Up vs Number of Cores')

        axes[0, 0].set_ylabel('Avg. Iterations')
        axes[0, 1].set_ylabel('Avg. BER')
        axes[1, 0].set_ylabel('Avg. Time (s)')
        axes[1, 1].set_ylabel('Speed Up')
        axes[2, 0].set_ylabel('Avg. Time (s)')
        axes[2, 1].set_ylabel('Speed Up')
        ...
        for t in range(0, i + 1):
            ...
            for l in range(0, len(itr_label)):
                ...
                if l > 0:
                    ...
                    spu[l - 1] = spu[l - 1] + qrT[t][0][j] / min(qrT[t][l][j], qrT[t][l + 1][j])
        ...
        for x in range(0, 3):  # init
            ...
            for l in range(0, len(itr_label)):
                omp_itr[l] = omp_itr[l] / i
                omp_ber[l] = omp_ber[l] / i
                omp_red[l] = omp_red[l] / i
                omp_stm[l] = omp_stm[l] / i
                omp_total[l] = omp_total[l] / i
                if l < len(itr_label) - 1:
                    omp_spu[l] = omp_spu[l] / i
                    omp_spu2[l] = omp_spu2[l] / i

            if j == 0:
                axes[0, 0].plot(itr_label, omp_itr, color=color[x], marker=marker[x], label=labels[x], markersize=12)
            else:
                axes[0, 0].plot(itr_label, omp_itr, color=color[x], marker=marker[x], markersize=12)

            axes[0, 1].plot(itr_label, omp_ber, color=color[x], marker=marker[x], markersize=12)
            ...
            axes[2, 0].plot(itr_label, omp_total, color=color[x], marker=marker[x], markersize=12)
        ...
        fig.legend(bbox_to_anchor=(0.88, 0.97), title="Legend", ncol=4, fontsize=21, title_fontsize=21,
                   edgecolor='black')
        if j == 0:
            plt.savefig(f'./{n}_report_plot_BNP.png')
            plt.savefig(f'./{n}_report_plot_BNP.eps', format='eps', dpi=1200)
            plt.close()
...
if __name__ == "__main__":
    n = 512
    a = np.load(f'./test_result/{n}_report_plot_1_BNP.npz')
    i = a['i']
    max_proc = a['max_proc']
    min_proc = a['min_proc']
    qrT = a['qrT']
    ...
    ber=a['ber']
    itr=a['itr']
    plot_bnp(n, i, max_proc, min_proc, qrT, asplT, bnp, ber, itr)
