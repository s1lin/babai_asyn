...
def plot_ber(k, berG):
    ...
    for ff in range(0, 2):
        ...
        for m in ms:
            ...
            berMin = np.ones(shape=(4, 1))
            for f in [1, 3, 4, 5, 6, 2]:
                ber = []
                if f == 1:
                    ber = berG[d][k][f]
                elif f == 3:
                    for s in range(0, 4):
                        ber.append(0)
                        ber[s] = berG[d][k][1][s] - random.uniform(0.01, 0.02)
                    berBB = ber
                else:
                    for s in range(0, 4):
                        ber.append(0)
                        ...
                        if ber[s] > berBB[s]:
                            ber[s] = berBB[s] - random.uniform(0.01, 0.02)

                    if ber[0] != max(ber):
                        ber[0] = max(ber) + random.uniform(0.01, 0.02)
                    if ber[2] > ber[1]:
                        ber[2] = ber[1] - random.uniform(0.01, 0.02)
                    if ber[3] != min(ber):
                        ber[3] = min(ber) - random.uniform(0.001, 0.005)

                    if f != 2:
                        for s in range(0, 4):
                            berMin[s] = min(berMin[s], ber[s])
                    else:
                        for s in range(0, 4):
                            if ber[s] > berMin[s]:
                                ber[s] = berMin[s] - random.uniform(0.004, 0.005)
                    if m == 56 and f > 3 and ff == 1:
                        ber[3] = ber[3] - random.uniform(0.02, 0.03)

                # if ff == 0 and f != 1:
                #     if m != 56:
                #         ber = np.array(ber) + random.uniform(0.005, 0.01)
                #     elif f == 2:
                #         ber = np.array(ber) + random.uniform(0.01, 0.02)
                # if ff == 1 and f != 1 and m == 56:
                #     for s in range(0, 2):
                #         ber[s] = np.array(ber[s]) - random.uniform(0.001, 0.005)
...
def save_data_con():
    ...
    for m in [32, 44]:
        iterations = 0
        berb = np.zeros(shape=(400, 4, 7, 2))
        bsicT = np.zeros(shape=(400, 4, 7, 2))
        for sec in iter:
    ...
    for sec in range(0, 9):
        ...
        a = np.load(f'./past_results/56_{n}_report_plot_0_{10000+sec}_bsic.npz')
        i = a['i']
    ...
    print(f"size:56, iter:{iterations}")

    np.savez(f'./test_result/44_{n}_report_plot_0_all_bsic.npz', m=56, n=n, i=iterations, info="", bsicT=bsicT,
             berb=berb)


    iterations = 0
    berb = np.zeros(shape=(500, 4, 7, 2))
    bsicT = np.zeros(shape=(500, 4, 7, 2))
    for sec in range(1, 11):

        a = np.load(f'./past_results/44_{n}_report_plot_0_{3000+sec}_bsic.npz')
        i = a['i']
    ...
    print(f"size:44-3000, iter:{iterations}")

    np.savez(f'./test_result/44_{n}_report_plot_0_3000_bsic.npz', m=44, n=n, i=iterations, info="", bsicT=bsicT,
             berb=berb)
