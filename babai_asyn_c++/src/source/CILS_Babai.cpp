...
namespace cils {
    ...
    public:
        ...
        cils_babai_method_omp(const b_matrix &R, const b_vector &y_bar, const index n_proc, const index nswp, const index init) {
            ...
            z_hat.clear();
            for (index ii = 0; ii < z_hat.size(); ii++){
                z_hat[ii] = init;
            }
...
//            ber[0] = helper::find_bit_error_rate<scalar, index>(z_ser, z_hat, cils.qam);
