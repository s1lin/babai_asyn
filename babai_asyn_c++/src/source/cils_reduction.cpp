...
//               ser_time, par_time, ser_time / par_time);
    ...
    scalar cils<scalar, index, n>::cils_LLL_serial() {
        ...
        coder::array<scalar, 1U> swap, vr;
        coder::array<scalar, 2U> b, b_R, r1;
        scalar G[4], low_tmp[2], zeta, r_ii, alpha, s;
        index c_result[2], sizes[2], i, i1, ci2, c_tmp, tmp, b_i, i2, row_i1, row_i2, c_i;
...
//                        b_loop_ub = r1.size(1);
            ...
            even = !even;
            i = even ? i : static_cast<int>((n + -1.0) / 2.0);
        }
        time = omp_get_wtime() - time;
        return time;
    }
