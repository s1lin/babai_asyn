...
#pragma omp simd reduction(+ : d)
    ...
    public:
        ...
        */
        ...
        /**
         * Parallel version of QR-factorization using modified Gram-Schmidt algorithm with locks
         * @param n_proc
         * @return
         */
        void cils_qr(const index n_proc) {

            index i, j, k, m, thrid;
            index qr_eval = 1, eval = 1; //Debuging FLAGS
            scalar error, d, time, s_time, t[n * n], c[n * n], r_sum = 0;
            omp_lock_t *lock;
            ...
            time = omp_get_wtime();
#pragma omp parallel default(shared) num_threads(n_proc) private(i, j, k, r_sum, thrid)
            {
...
#pragma omp for schedule(static, 1)
                ...
                if (thrid == 0) {
                    // Calculation of ||A||
                    for (i = 0; i < n; i++) {
                        r_sum = r_sum + A->x[0 * n + i] * A->x[0 * n + i];
                    }
                    R->x[0 * n + 0] = sqrt(r_sum);
                    for (i = 0; i < n; i++) {
                        Q->x[0 * n + i] = A->x[0 * n + i] / R->x[0 * n + 0];
                    }
...
#pragma omp for schedule(static, 1) nowait
                    for (j = 0; j < n; j++) {
                        if (j >= k) {
                            R->x[(k - 1) * n + j] = 0;
                            for (i = 0; i < n; i++) {
                                R->x[j * n + (k - 1)] += Q->x[(k - 1) * n + i] * A->x[j * n + i];
                            }
                            for (i = 0; i < n; i++) {
                                A->x[j * n + i] = A->x[j * n + i] - R->x[j * n + (k - 1)] * Q->x[(k - 1) * n + i];
                            ...
                            if (j == k) {
                                ...
                                r_sum = 0;
                                for (i = 0; i < n; i++) {
                                    r_sum = r_sum + A->x[k * n + i] * A->x[k * n + i];
                                }
                                R->x[k * n + k] = sqrt(r_sum);

                                //#pragma omp for schedule(static,1) nowait
                                for (i = 0; i < n; i++) {
                                    Q->x[k * n + i] = A->x[k * n + i] / R->x[k * n + k];
                                }
