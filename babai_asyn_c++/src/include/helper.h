...
namespace helper {
    ...
    static void eye(std::array<scalar, n * n> &A) {
        A.fill(0);
        for (index i = 0; i < n; i++) {
            A[i + n * i] = 1.0;
        }
    ...
    static void b_mtimes(const array<scalar, m * n> &A_C, const array<scalar, n * mb> &B, array<scalar, m * mb> &C) {
        C.fill(0);
    ...
    template<typename scalar, typename index, index m, index n>
    static void mtimes_1(const array<scalar, m * n> &Q, const array<scalar, n> &R, array<scalar, m> &A_t) {
        A_t.fill(0);
        for (index k = 0; k < n; k++) {
            for (index i = 0; i < m; i++) {
                A_t[i] += Q[k * m + i] * R[k];
    ...
    template<typename scalar, typename index, index n>
    static void vsubtract(const array<scalar, n> &x, const scalar *y, array<scalar, n> &z){
        for (index i = 0; i < n; i++){
            z[i] = x[i] - y[i];
        }
    }
    ...
    template<typename scalar, typename index, index m, index n, index mb>
    static void mrdiv(array<scalar, m * n> &B, const array<scalar, n * mb> &A) {
